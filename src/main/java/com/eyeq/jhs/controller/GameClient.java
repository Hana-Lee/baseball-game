					...
					case 1:
						...
						if (gameRoomNumScanner.hasNextLine()) {
							...
							if (userNameScanner.hasNextLine()) {
								final String userName = userNameScanner.nextLine();
								client.sendSocketData("JOIN," + gameRoomNum + ":USER:" + userName);
								System.out.println("안녕하세요 " + userName + "님, " + gameRoomNum + "번 방에 입장하셨습니다");

								boolean gameRoomLeft = false;
								while (!gameRoomLeft) {
									client.sendSocketData("GET_ROOM_LIST");
									gameRoomList = objectMapper.readValue(client.getServerMessage(), objectMapper
											.getTypeFactory().constructCollectionType(List.class, GameRoom.class));
											...
											case 1:
												...
												while (!isGameOver) {
													...
													if (s2.hasNextLine()) {
														...
														try {
															ResultDto resultDto = objectMapper.readValue(serverMsg,
																	ResultDto.class);
															System.out.println("Result : " + resultDto);

															if (resultDto.getErrorMessage() != null) {
																System.out.println("오류 메세지 : " + resultDto
																		.getErrorMessage().getType().getMessage());
															} else {
																System.out.println(resultDto.getResult().getStrike()
																		.getValue() + "스트라이크, " +
																		resultDto.getResult().getBall().getValue() +
																		"볼 입니다.");
															}

															if (resultDto.getResult().getSolve().isValue()) {
																System.out.println("축하합니다. 숫자를 맞추셨네요 ^^");
																System.out.println("점수는 : " + resultDto.getScore()
																		.getValue() + "점 입니다.");
																isGameOver = true;
															}
														} catch (IOException e) {
														...
														switch (settingInput.nextInt()) {
															case 1:
																...
																if (inputSettingNum.hasNextInt()) {
																	//setting.setLimitWrongInputCount(inputSettingNum
																	// .nextInt());
																}
																break;
															case 2:
																...
																if (inputSettingNum2.hasNextInt()) {
																	//setting.setLimitGuessInputCount(inputSettingNum2
																	// .nextInt());
																}
																break;
															case 0:
